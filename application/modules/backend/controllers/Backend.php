...
class Backend extends MX_Controller
{
	...
	public function edit_catalogue()
	{
		...
		$this->data["catalogue"] = $this->db->select("a.*, b.name as category_name")->from("catalogue_item a")->join("catalogue_category b", "b.id = a.category_id", "LEFT")->where("a.id", $this->input->get("id"))->get()->row();
