		...
		if($captcha_s == $captcha){
			...
			if($dout['baris'] == 1 OR $drma['baris'] == 1){
				...
				if($dout['baris'] == 1){ $bulan = $dout['garansi_out'];
					...
					$tgl_beli = formattgl($dout['tgl_keluar']); $idpem = $dout['id_customer'];
					...
					$tgl_beli = formattgl($drma['tgl_invoice_sor']); $idpem = $drma['id_pembeli_sor'];
				}
				$sisa = strtotime($tgl_batas) - strtotime(date("Y-m-d"));
				$tgl_batas2 = new DateTime($tgl_batas); $diff = $tgl_now->diff($tgl_batas2);
				if($sisa >= 0){
					$tgl_batas1 = $tgl_batas; $str_garansi = "<span class='label label-success' style='font-size:16px;'>Masih Garansi</span>";
