...
(function($, window, document, undefined) {
  ...
  WanSpinner.prototype.bind = function() {
    var self = this;
    $(self.element).delegate('.TKurangSpin', 'click', function() {
      ...
      if (val < self.options.minValue) {
        ...
        input.val(val);
        self.options.minusClickEnable && self.options.minusClick($(this).parent(), val, idproduk);
        //self.options.valueChangedEnable && self.options.valueChanged($(this).parent(), val, idproduk);
      }
      return false;
    }).delegate('.TTambahSpin', 'click', function() {
      var idproduk = $(this).data('idproduk');
      var val;
      var input = $(this).siblings('input');
      if (self.options.stepFloat > 1) {
      ...
      if (val > self.options.maxValue) {
        ...
        input.val(val);
        self.options.plusClickEnable && self.options.plusClick($(this).parent(), val, idproduk);
        //self.options.valueChangedEnable && self.options.valueChanged($(this).parent(), val, idproduk);
      }
      return false;
      ...
      if (val > self.options.maxValue) {
        ...
        self.options.exceptionFunEnable && self.options.exceptionFun($(this).parent(), self.EXCEPTION.TOO_LARGE, idproduk);
      } else if (val < self.options.minValue) {
        val = self.options.minValue;
        self.options.exceptionFunEnable && self.options.exceptionFun($(this).parent(), self.EXCEPTION.TOO_SMALL, idproduk);
      }
      $(this).val(val);
      self.options.valueChangedEnable && self.options.valueChanged($(this).parent(), val, idproduk);
    });
